...
public:
	...
	Vec3 GetXAxis() const
	{
		return Vec3::Normalize(Vec3(mat[0], mat[1], mat[2]));
	}

	Vec3 GetYAxis() const
	{
		return Vec3::Normalize(Vec3(mat[4], mat[5], mat[6]));
	}

	Vec3 GetZAxis() const
	{
		return Vec3::Normalize(Vec3(mat[8], mat[9], mat[10]));
	}

	Vec3 GetScale() const
	{
		Vec3 retVal;
		retVal.x = Vec3(mat[0], mat[1], mat[2]).Length();
		retVal.y = Vec3(mat[4], mat[5], mat[6]).Length();
		retVal.z = Vec3(mat[8], mat[9], mat[10]).Length();
		return retVal;
	}
	...
	static Matrix4 CreateLookAt(const Vec3& eye, const Vec3& target, const Vec3& up)
	{
		...
		Vec3 normalizedUp = Vec3::Normalize(up);
		Vec3 xaxis = Vec3::Normalize(Vec3::Cross(normalizedUp, zaxis));
		...
		Matrix4 translation = Matrix4::CreateTranslation(Vec3(-eye.x, -eye.y, -eye.z));
